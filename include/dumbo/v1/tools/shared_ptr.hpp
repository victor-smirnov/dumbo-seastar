 ...
 * Copyright (C) 2016 Victor Smirnov.
 */
...
protected:
    ...
    // FIXME: figure out what to do with cpu_id_ in these constructors. And for shared_ptr too.
...
public:
    ...
    enable_lw_shared_from_this() noexcept : cpu_id_(engine().cpu_id()) {}
    enable_lw_shared_from_this(int cpu_id) noexcept : cpu_id_(cpu_id)  {}
...
public:
    ...
    shared_ptr_no_esft(): cpu_id_(engine().cpu_id()) {}
    shared_ptr_no_esft(int cpu_id): cpu_id_(cpu_id) {}
...
struct shared_ptr_count_base {
    ...
    shared_ptr_count_base(): cpu_id_(engine().cpu_id()) {}
    shared_ptr_count_base(int cpu_id): cpu_id_(cpu_id) {}
...
private:
    explicit shared_ptr_holder(shared_ptr_count_for<T>* b) noexcept : _b(b), _p(&b->data) {
        ++_b->count;
    }
    shared_ptr_holder(shared_ptr_count_base* b, T* p) noexcept : _b(b), _p(p) {
        if (_b) {
            ++_b->count;
        }
    }
    explicit shared_ptr_holder(enable_shared_from_this<std::remove_const_t<T>>* p) noexcept : _b(p), _p(static_cast<T*>(p)) {
        if (_b) {
            ++_b->count;
        }
    }
public:
    ...
    shared_ptr_holder(shared_ptr_holder<T>&& other) noexcept : _b(other._b), _p(other._p) {
    	other._b = nullptr;
    	other._p = nullptr;
    }

    void operator=(shared_ptr_holder&& other) noexcept {
        if (this != &other)
        {
        	_b = other._b;
        	_p = other._p;
...
public:
    ...
    shared_ptr(shared_ptr_holder<T>&& holder): _b(holder._b), _p(holder._p)
	{
    	holder._b = nullptr;
    	holder._p = nullptr;
...
struct shared_ptr_make_helper<T, false> {
    ...
    template <typename... A>
    static shared_ptr_holder<T> make_holder(A&&... a) {
    	return shared_ptr_holder<T>(new shared_ptr_count_for<T>(std::forward<A>(a)...));
    }
};
...
struct shared_ptr_make_helper<T, true> {
    ...
    static shared_ptr_holder<T> make_holder(A&&... a) {
    	auto p = new T(std::forward<A>(a)...);
    	return shared_ptr_holder<T>(p, p);
    }
};
...
inline
shared_ptr<T>
make_shared_holder(T&& a) {
    using helper = shared_ptr_make_helper<T, std::is_base_of<shared_ptr_count_base, T>::value>;
    return helper::make_holder(std::forward<T>(a));
}
...
template <typename T>
struct hash<dumbo::v1::dumbo::lw_shared_ptr<T>> : private hash<T*> {
    size_t operator()(const dumbo::v1::dumbo::lw_shared_ptr<T>& p) const {
        return hash<T*>::operator()(p.get());
    }
};

template <typename T>
struct hash<dumbo::v1::dumbo::shared_ptr<T>> : private hash<T*> {
    size_t operator()(const dumbo::v1::dumbo::shared_ptr<T>& p) const {
        return hash<T*>::operator()(p.get());
    }
};
...
template<typename T>
struct is_smart_ptr<dumbo::v1::dumbo::shared_ptr<T>> : std::true_type {};

template<typename T>
struct is_smart_ptr<dumbo::v1::dumbo::lw_shared_ptr<T>> : std::true_type {};
